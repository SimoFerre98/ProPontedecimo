...
| `profiles` | US-002 (RLS), US-005 (initial schema), US-018 (sync email trigger + self name update), US-019 (password recovery), US-020 (role assignment) | `trg_enforce_role_change` blinda la scrittura del `role` a livello DB limitandola a `president`. Le modifiche anagrafiche (`full_name`) devono essere permesse in self-update (policy `profiles_update_self`). Il trigger di sync email `on_auth_user_email_updated` agisce come `SECURITY DEFINER` per allineare l'email dopo conferma. Per US-019/US-020 verificare che le nuove operazioni su ruoli o password non collidano con le policy e i trigger attuali. |
| `parent_players` / `get_parent_player_ids()` | US-002 (tabella + 4 policy RLS), US-027 (enum `parent_link_status`, colonna `status`, policy `parent_players_insert_self_request`, RPC `search_players_for_parent_request`/`get_my_parent_players`) | **Invariante critica:** `get_parent_player_ids()` filtra `status = 'confirmed'`: le quattro policy `players_select_parent`, `medical_select_parent`, `payments_select_parent`, `attendance_select_parent` dipendono tutte da questa funzione. Qualunque story che scriva su `parent_players` deve preservare questo invariante e non portare mai righe `pending` a risultare visibili tramite quelle policy. L'unica policy che permette l'INSERT al genitore è `parent_players_insert_self_request` (solo `status = 'pending'`, solo `parent_profile_id = auth.uid()`). |
| `events` (colonne `opponent`, `team_sector`, `call_up_published_at`) | US-012/US-013 (tabella base, tipi evento, doppio orario), US-032 (colonne aggiunte, nessun vincolo NOT NULL/CHECK) | Le tre colonne sono nullable di proposito: obbligatorie solo nel flusso applicativo "crea partita per la convocazione", non a livello DB. `US-030` (visualizzazione) e `US-031` (calendario/classifica leva) le riusano così come sono — non serve una nuova migrazione su `events` per quelle due story. `events_all_coach` resta senza scoping per leva (gap preesistente da US-013): `publishCallUps`/`unpublishCallUps` vincolano "solo fino al ritrovo" solo in UI, non via RLS. |
| `call_ups` | US-032 (tabella, RLS coach/admin/player, riusa `is_coach_of_player` da US-017) | Nessuna colonna di stato: la presenza della riga è "convocato". Vincolo di unicità `(event_id, player_id)`. Scrittura (`INSERT`/`DELETE`) vincolata sia per leva (`is_coach_of_player`) sia per tempo (`events.meetup_time > now()`) a livello RLS; la lettura del coach non ha invece limiti di tempo. La visibilità del giocatore (`call_ups_select_player`) dipende da `events.call_up_published_at IS NOT NULL`, verificato tramite `is_call_up_published(event_id)` (SECURITY DEFINER) e non con una sottoquery diretta su `events` — **il ruolo `player` non ha alcuna policy SELECT su `events`**, quindi qualunque nuova policy/RPC che debba leggere `events` per conto di un giocatore deve bypassare la RLS in modo esplicito (stesso motivo/pattern di `is_coach_of_player`), altrimenti risulterà sempre falsa in silenzio. Qualunque story che tocchi questa tabella deve preservare l'invariante `call_up_published_at`. **Nessuna policy `_select_parent` esiste ancora** — a differenza di `attendance`/`payments`/`medical_visits`, non è stata richiesta dagli AC di US-032; da valutare se una story futura la richiederà. |
