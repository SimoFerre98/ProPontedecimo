...
| `events` (colonne `opponent`, `team_sector`, `call_up_published_at`) | US-012/US-013 (tabella base, tipi evento, doppio orario), US-032 (colonne aggiunte), US-030 (visualizzazione convocazioni via RPC) | Le tre colonne sono nullable di proposito. Il ruolo `player` non possiede alcuna policy SELECT su `events`; US-030 introduce la RPC SECURITY DEFINER `get_my_next_call_up()` per consentire la lettura mirata dei soli dati della propria prossima partita pubblicata. Questo pattern fa da riferimento per US-031 (calendario leva). |
