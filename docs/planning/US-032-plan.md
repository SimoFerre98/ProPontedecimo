...
- **RLS a policy separate per comando** (non una `FOR ALL` unica), perché il vincolo temporale si applica solo a scrittura/cancellazione, non a lettura:
  ...
  - `call_ups_select_player` (per l'AC di US-030, non di questa story, ma appartiene alla stessa migrazione: il giocatore vede solo le proprie righe, e solo se `events.call_up_published_at IS NOT NULL`)
...
|---|---|---|---|---|---|
| TODO | TASK-01 | Migrazione: colonne `events` e tabella `call_ups` | Aggiunta `opponent`, `team_sector`, `call_up_published_at` a `events`; nuova tabella `call_ups` con vincolo di unicità `(event_id, player_id)`; RLS separate per comando (select/insert/delete coach, all admin, select player) riusando `is_coach_of_player` | Impl | - |
| TODO | TASK-02 | Test integrazione RLS | Nuovo `scripts/test-call-ups.mjs`: isolamento per leva, blocco post-ritrovo, visibilità giocatore solo dopo pubblicazione, accesso libero per president/director | Test | TASK-01 |
| TODO | TASK-03 | Rigenerazione tipi TypeScript | Rigenerare `src/types/database.ts` (tabella `call_ups`, colonne aggiunte a `events`) | Impl | TASK-01 |
| TODO | TASK-04 | Service layer `callUpService.ts` | `getUpcomingMatchEvents`, `getRosterForEvent`, `getCallUpsForEvent`, `toggleCallUp`, `publishCallUps`, `unpublishCallUps` | Impl | TASK-03 |
| TODO | TASK-05 | Estensione form evento partita | Aggiungere campi avversario e leva al modale di creazione/modifica evento quando `event_type` è `home_match`/`away_match` | Impl | TASK-03 |
| TODO | TASK-06 | Pagina `Convocazioni.tsx` | Selettore partita (scoping leva per il coach), lista atleti con toggle convocato/non convocato in stile `Attendance.tsx`, badge stato bozza/pubblicato, bottone Pubblica/Ritira pubblicazione, disabilitazione post-ritrovo | Impl | TASK-04 |
| TODO | TASK-07 | Route e voce di navigazione | Aggiungere `/convocazioni` in `App.tsx`, `NAV_ITEMS` e filtro di visibilità coach in `DashboardLayout.tsx` | Impl | TASK-06 |
| TODO | TASK-08 | Aggiornamento CLAUDE.md — Superfici condivise | Aggiungere `events` (nuove colonne) e `call_ups` alla tabella, indicando che US-030/US-031 dipendono da queste colonne e che nessuna policy `_select_parent` esiste ancora su `call_ups` | Impl | TASK-01 |
| TODO | TASK-09 | Verifica manuale end-to-end multi-ruolo | Coach crea partita → convoca → pubblica → modifica dopo pubblicazione; verifica blocco dopo ritrovo; president/director senza restrizioni di leva | Test | TASK-06, TASK-07 |
| TODO | TASK-10 | Regressione completa | `npx supabase db reset` + `npm run test:integration` (in particolare `test-rls.mjs` e i test calendario) + `npx tsc --noEmit` | Test | TASK-02, TASK-06, TASK-07 |
