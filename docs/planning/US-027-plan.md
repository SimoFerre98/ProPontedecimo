...
- **Documentazione**: aggiungere `parent_players` / `get_parent_player_ids()` alla tabella "Superfici condivise" di CLAUDE.md, indicando che le quattro policy `*_select_parent` dipendono dal filtro `status = 'confirmed'` — così una futura story (US-028, US-046 ex US-029) che tocca la stessa superficie parte già sapendo dell'invariante.
