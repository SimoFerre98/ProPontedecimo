...
- `PrimiPassiChapter.tsx` contiene testo semplice (niente terminologia tecnica) più un'immagine statica in `public/guide/primi-passi-panoramica.png`: uno screenshot reale dell'interfaccia (Dashboard) con callout numerati che indicano header, selettore stagione, menu utente/Guida, pulsante Menu flottante — l'immagine va catturata dall'app in esecuzione durante l'implementazione (TASK-01), non è generabile a priori.
...
|---|---|---|---|---|---|
| TODO | TASK-01 | Registro capitoli e capitolo "Primi passi" | Creare `src/data/guideChapters.tsx` con il registro (1 capitolo `available` + i futuri come `coming-soon`) e `src/components/guide/chapters/PrimiPassiChapter.tsx`; catturare uno screenshot reale della Dashboard, annotarlo con callout numerati e salvarlo in `public/guide/primi-passi-panoramica.png` | Impl | - |
| TODO | TASK-02 | Pagina `Guide.tsx` | Creare `src/pages/Guide.tsx`: colonna indice capitoli (attivo/disponibile vs coming-soon) + colonna contenuto capitolo attivo, stack verticale sotto `sm`, nessun reload al cambio capitolo | Impl | TASK-01 |
| TODO | TASK-03 | Voce "Guida" nei due dropdown e routing | Aggiungere il bottone "Guida" in `DashboardLayout.tsx` e `PortalLayout.tsx` (icona `BookOpen`, visibile a ogni ruolo) e registrare le rotte `guida` nel branch Staff e nel branch Portale in `App.tsx` | Impl | TASK-02 |
...
| TODO | TASK-05 | Verifica manuale multi-ruolo e responsive | Verificare su desktop e mobile, per almeno un ruolo Staff e uno Portale, apertura del dropdown, navigazione alla Guida e leggibilità del capitolo "Primi passi" | Test | TASK-03 |
